...
func AnalyzeWaitGroupUsage(errorCollector *report.ErrorCollector, fn *ast.FuncDecl, wgNames map[string]bool) map[string]*waitGroupStats {
	...
	// Traverse AST with stack to track for loop context (mantén la lógica original)
	...
	// Detección Add después de Wait en goroutines
	...
	// Verificar Add vs Wait en el mismo flujo (no en goroutines)
	for wgName, st := range stats {
	...
	// Chequeo Add sin Done en goroutines bloqueantes
